...
const UserManagement = () => {
    ...
    const handleDeleteUser = (userId) => {
        if (window.confirm("Are you sire you want to delete the user?")) {
                    ...
                    <tbody >
                        {users?.map((user) => (
                            <tr key={user._id} className='border-b hover:bg-gray-50'>
                                ...
                                <td className="p-4 ">
                                    <select value={user.role} id="" onChange={(e) => { handleRoleChange(user._id, e.target.value) }} className='p-2 border rounded'>
                                        <option value="customer">Customer</option>
                                        <option value="admin">Admin</option>
                                    </select>
                                </td>
                                <td className="p-4 ">
                                    <button onClick={() => handleDeleteUser({ id: user?._id })} className='bg-red-500 text-white px-4 py-2 hover:bg-red-600'>Delete</button>
                                </td>
                            </tr>
                        ))}
                    </tbody>
